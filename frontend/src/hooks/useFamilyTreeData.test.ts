 ...
 */
const personRelationshipPublicArbitrary = (relType: RelationshipType) =>
  fc.record({
    id: fc.uuid(),
    person_id: fc.uuid(),
      ...
      fc.assert(
        fc.property(
          ...
          (category, count) => {
            ...
            const relationships: PersonRelationshipWithDetails[] = Array.from(
              { length: count },
              () => ({
                relationship: {
      ...
      fc.assert(
        fc.asyncProperty(
          ...
          async (selectedPersonId, parentIds) => {
            ...
            expect(
              result.every((sibling) => sibling.id !== selectedPersonId),
            ).toBe(true)
          },
        ),
        { numRuns: 100 },
