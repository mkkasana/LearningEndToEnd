...
const personDetailsArbitrary = fc.record({
  ...
  created_at: fc
    .integer({ min: 1609459200000, max: Date.now() })
            ...
            (parents) => {
              ...
              for (const parent of uniqueParents) {
                ...
                const nameElements = Array.from(
                  container.querySelectorAll(".font-semibold"),
                )
                const hasName = nameElements.some(
        ...
        fc.assert(
          fc.property(
            ...
            (parent1, parent2) => {
              ...
              const hasFather = nameElements.some(
                (el) => el.textContent?.trim() === fatherName,
              )
              const hasMother = nameElements.some(
                (el) => el.textContent?.trim() === motherName,
              )
        ...
        fc.assert(
          fc.property(
            ...
            (spouses) => {
              ...
              const displayName =
                `${firstSpouse.first_name} ${firstSpouse.last_name}`.trim()
              const nameElements = Array.from(
                container.querySelectorAll(".font-semibold"),
              )
              const hasName = nameElements.some(
