...
const personDetailsArbitrary = fc.record({
  ...
  middle_name: fc.option(fc.string({ minLength: 1, maxLength: 20 }), {
    nil: null,
  }),
...
describe("HorizontalScrollRow", () => {
  describe("Unit Tests", () => {
    ...
    it("should maintain scrollability when content is wide", () => {
      const manyPeople: PersonDetails[] = Array.from(
        { length: 15 },
        (_, i) => ({
          id: `person-${i}`,
          first_name: `Person`,
        ...
        fc.property(
          ...
          (people, variant) => {
            ...
            const hasTouchSupport =
              container.innerHTML.includes("touch-manipulation")
  ...
  describe("Property 12: Selected Person Centering", () => {
      ...
      fc.assert(
        fc.property(
          ...
          (siblings, selectedPerson, spouses) => {
            ...
            expect(personCards.length).toBe(
              siblings.length + 1 + spouses.length,
            )
    ...
    it("should handle centering when selected person has no siblings or spouses", () => {
      fc.assert(
        fc.property(personDetailsArbitrary, (selectedPerson) => {
          const mockOnClick = vi.fn()
        ...
        fc.property(
          ...
          (parents, siblings, selectedPerson, spouses, children) => {
            ...
            if (parents.length > 0) {
              ...
              const parentCards =
                parentContainer.querySelectorAll('[role="button"]')
              expect(parentCards.length).toBe(parents.length)

              // Parent row should exist
              const parentRegion =
                parentContainer.querySelector('[role="region"]')
              expect(parentRegion?.getAttribute("aria-label")).toBe(
                "Parents row",
              )
            }
            ...
            const centerCards =
              centerContainer.querySelectorAll('[role="button"]')
            expect(centerCards.length).toBe(centerPeople.length)

            // Check for sibling color coding (blue)
            if (siblings.length > 0) {
              const hasSiblingColor =
                centerContainer.innerHTML.includes("blue-")
              expect(hasSiblingColor).toBe(true)
            }

            // Check for spouse color coding (purple)
            if (spouses.length > 0) {
              const hasSpouseColor =
                centerContainer.innerHTML.includes("purple-")
              expect(hasSpouseColor).toBe(true)
            }
            ...
            const centerRegion =
              centerContainer.querySelector('[role="region"]')
            expect(centerRegion?.getAttribute("aria-label")).toBe(
              "Center row with siblings and spouses",
            )
            ...
            if (children.length > 0) {
              ...
              const childRegion =
                childContainer.querySelector('[role="region"]')
              expect(childRegion?.getAttribute("aria-label")).toBe(
                "Children row",
              )
            }
          },
      ...
      fc.assert(
        fc.property(
          ...
          (people) => {
            ...
            const parentHasItemsCenter =
              parentContainer.innerHTML.includes("items-center")
            const childHasItemsCenter =
              childContainer.innerHTML.includes("items-center")
