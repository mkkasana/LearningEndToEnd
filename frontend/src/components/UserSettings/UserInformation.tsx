...
const UserInformation = () => {
  ...
  const getReligionName = () => {
    ...
    const religion = religions.find((r: any) => r.religionId === religionData.religion_id)
    return religion?.religionName || "N/A"
  }

  const getCategoryName = () => {
    if (!religionData?.religion_category_id || !categories) return null
    const category = categories.find((c: any) => c.categoryId === religionData.religion_category_id)
    return category?.categoryName
  }

  const getSubCategoryName = () => {
    if (!religionData?.religion_sub_category_id || !subCategories) return null
    const subCategory = subCategories.find((sc: any) => sc.subCategoryId === religionData.religion_sub_category_id)
  ...
  const getAddressDisplay = () => {
    ...
    if (currentAddress.locality_id && localities) {
      const locality = localities.find((l: any) => l.localityId === currentAddress.locality_id)
      if (locality) parts.push(locality.localityName)
    }
    
    if (currentAddress.sub_district_id && subDistricts) {
      const subDistrict = subDistricts.find((sd: any) => sd.tehsilId === currentAddress.sub_district_id)
      if (subDistrict) parts.push(subDistrict.tehsilName)
    }
    
    if (currentAddress.district_id && districts) {
      const district = districts.find((d: any) => d.districtId === currentAddress.district_id)
      if (district) parts.push(district.districtName)
    }
    
    if (currentAddress.state_id && states) {
      const state = states.find((s: any) => s.stateId === currentAddress.state_id)
      if (state) parts.push(state.stateName)
    }
    
    if (currentAddress.country_id && countries) {
      const country = countries.find((c: any) => c.countryId === currentAddress.country_id)
