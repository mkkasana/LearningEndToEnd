...
- [x] 2. Update User model and schemas
  - [x] 2.1 Update `backend/app/db_models/user.py`
    ...
    - Add `@property is_superuser` for backward compatibility (returns True if role is SUPERUSER or ADMIN)
...
- [x] 6. Add role management API endpoint
  ...
  - [ ]* 6.2 Write property test for admin role management
...
- [ ]* 7. Checkpoint - Verify API changes
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 8. Create database migration
  ...
  - [-]* 8.2 Write property test for migration role conversion
...
- [x] 9. Update existing route dependencies
  - [x] 9.1 Review and update routes using `get_current_active_superuser`
    - Verify existing superuser-protected routes still work with new system
    - Update any routes that should require admin-only access
