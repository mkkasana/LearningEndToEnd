...
class PersonProfessionService:
    ...
    def create_profession(
        ...
        logger.info(f"Creating profession for person ID: {person_id}, is_current={profession_create.is_current}")
        ...
        logger.info(f"Profession created successfully: ID {created_profession.id} for person {person_id}")
    ...
    def update_profession(
        ...
        """Update a profession."""
        logger.info(f"Updating profession: ID {profession.id} for person {profession.person_id}")
        update_data = profession_update.model_dump(exclude_unset=True)
        
        # Log what fields are being updated
        update_fields = list(update_data.keys())
        if update_fields:
            logger.debug(f"Updating fields for profession {profession.id}: {update_fields}")

        # If marking as current, clear other current professions
        if update_data.get("is_current"):
            logger.debug(f"Clearing other current professions for person {profession.person_id}")
    ...
    def delete_profession(self, profession: PersonProfession) -> None:
        """Delete a profession."""
        logger.warning(f"Deleting profession: ID {profession.id} for person {profession.person_id}")
