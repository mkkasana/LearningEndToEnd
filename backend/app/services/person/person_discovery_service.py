...
class PersonDiscoveryService:
    ...
    def _discover_spouses_children(
        ...
        spouse_relationships = [
            ...
            if r.relationship_type in [RelationshipType.WIFE, RelationshipType.HUSBAND, RelationshipType.SPOUSE]
        ]
        
        logger.debug(f"Found {len(spouse_relationships)} spouse(s) for person {person_id}")
        ...
        for spouse_rel in spouse_relationships:
            ...
            logger.debug(f"Checking children of spouse: {spouse_name} (ID: {spouse_id})")
            ...
            logger.debug(f"Spouse {spouse_name} has {len(child_relationships)} child(ren)")
            ...
            for child_rel in child_relationships:
                ...
                if child_id in connected_person_ids:
                    logger.debug(f"Child {child_id} already connected to user, skipping")
                    continue
                ...
                inferred_relationship_type = self._infer_child_relationship(child.gender_id)
    ...
    def _discover_parents_spouse(
        ...
        logger.debug(f"Found {len(parent_relationships)} parent(s) for person {person_id}")
        ...
        for parent_rel in parent_relationships:
            ...
            spouse_relationships = self.relationship_repo.get_by_relationship_types(
                parent_id,
                [RelationshipType.WIFE, RelationshipType.HUSBAND, RelationshipType.SPOUSE]
            )
            
            # Filter to active relationships only
            spouse_relationships = [r for r in spouse_relationships if r.is_active]
            
            logger.debug(f"Parent {parent_name} has {len(spouse_relationships)} spouse(s)")
            ...
            for spouse_rel in spouse_relationships:
                ...
                if spouse_id in connected_person_ids:
                    logger.debug(f"Spouse {spouse_id} already connected to user, skipping")
                    continue
                ...
                inferred_relationship_type = self._infer_parent_relationship(spouse.gender_id)
    ...
    def _discover_childs_parent(
        ...
        logger.debug(f"Found {len(child_relationships)} child(ren) for person {person_id}")
        ...
        for child_rel in child_relationships:
            ...
            logger.debug(f"Child {child_name} has {len(parent_relationships)} parent(s)")
            ...
            for parent_rel in parent_relationships:
                ...
                if parent_id in connected_person_ids:
                    logger.debug(f"Parent {parent_id} already connected to user, skipping")
                    continue
    ...
    def _sort_and_limit_discoveries(
        ...
        sorted_discoveries = sorted(
            deduplicated,
            key=lambda d: (d.proximity_score, d.relationship_priority, d.first_name.lower())
        )
