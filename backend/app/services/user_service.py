...
class UserService:
    ...
    def create_user(self, user_create: UserCreate) -> User:
        ...
        logger.info(
            ...
            f"role={created_user.role}, is_active={created_user.is_active}"
        )
        return created_user
    ...
    def update_user_role(self, user: User, new_role: UserRole) -> User:
        ...
        logger.info(
            ...
            f"from {user.role} to {new_role}"
        )
        user.role = new_role
        updated_user = self.user_repo.update(user)
        logger.info(
            f"Role updated successfully for user: {user.email} "
            f"(ID: {user.id}), new role: {updated_user.role}"
        )
        return updated_user
