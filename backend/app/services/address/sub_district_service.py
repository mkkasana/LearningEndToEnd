...
class SubDistrictService:
    ...
    def get_sub_districts_by_district(
        ...
        logger.debug(f"Found {len(sub_districts)} sub-districts for district {district_id}")
        return [
    ...
    def get_sub_district_by_id(self, sub_district_id: UUID) -> SubDistrict | None:
        ...
        if sub_district:
            logger.debug(f"Sub-district found: {sub_district.name} (ID: {sub_district_id})")
        else:
    ...
    def create_sub_district(self, sub_district_in: SubDistrictCreate) -> SubDistrict:
        ...
        logger.info(f"Creating sub-district: {sub_district_in.name} for district {sub_district_in.district_id}")
        ...
        logger.info(f"Sub-district created successfully: {created_sub_district.name} (ID: {created_sub_district.id})")
    ...
    def update_sub_district(
        ...
        logger.info(f"Updating sub-district: {sub_district.name} (ID: {sub_district.id})")
        update_data = sub_district_update.model_dump(exclude_unset=True)
        
        # Log what fields are being updated
        update_fields = list(update_data.keys())
        if update_fields:
            logger.debug(f"Updating fields for sub-district {sub_district.id}: {update_fields}")
        ...
        logger.info(f"Sub-district updated successfully: {updated_sub_district.name} (ID: {updated_sub_district.id})")
    ...
    def code_exists(
        ...
        if not code:
            return False
        logger.debug(f"Checking if sub-district code exists: {code} in district {district_id}")
    ...
    def delete_sub_district(self, sub_district: SubDistrict) -> None:
        ...
        logger.warning(f"Deleting sub-district: {sub_district.name} (ID: {sub_district.id})")
        self.sub_district_repo.delete(sub_district)
        logger.info(f"Sub-district deleted successfully: {sub_district.name} (ID: {sub_district.id})")
